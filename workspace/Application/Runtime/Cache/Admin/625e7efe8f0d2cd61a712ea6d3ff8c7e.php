                ...
                <div class="col-md-3">
                    <div class="input-group">
                        ...
                        <select class="form-control" name="position_id">
                            ...
                            <?php if(is_array($positions)): $i = 0; $__LIST__ = $positions;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ps): $mod = ($i % 2 );++$i;?><option value="<?php echo ($ps["id"]); ?>" ><?php echo ($ps["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-bordered table-hover singcms-table">
                        ...
                        <tbody class="text-center">
                          <?php if(is_array($contents)): $i = 0; $__LIST__ = $contents;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$new): $mod = ($i % 2 );++$i;?><tr>
                                <td><input size=4 type='text'  name='listorder[<?php echo ($new["id"]); ?>]' value="<?php echo ($new["listorder"]); ?>" class="text-center"/></td>
                                <td><?php echo ($new["id"]); ?></td>
                                <td><?php echo ($new["title"]); ?></td>
                                <td><?php echo (getPositionName($positions,$new["position_id"])); ?></td>
                                <td><?php echo (date("Y-m-d H:i",$new["create_time"])); ?></td>
                                <td> <?php echo (isThumb($new["thumb"])); ?></td>
                                <td>
                                    <span  attr-status="<?php if($new['status'] == 1): ?>0<?php else: ?>1<?php endif; ?>"  attr-id="<?php echo ($new["id"]); ?>" class="sing_cursor singcms-on-off" id="singcms-on-off" ><?php echo (getStatus($new["status"])); ?></span>
                                </td>
                                <td>
                                    <span class="sing_cursor glyphicon glyphicon-edit" aria-hidden="true" id="singcms-edit" attr-id="<?php echo ($new["id"]); ?>" ></span>
